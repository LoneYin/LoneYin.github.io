...
function FullPage(options) {
   var pageContain = document.getElementById(options.id),
      ...
      stepNow = [],
      mode = [],
      modeLen, navChildren, SPACE = " ",
      _interval = null,
      _isLocked = false,
      _isNav = false,
      _curve, _t, _fix, init, setCubic, trans, resetAndRun, checkStep, onTap, replaceClass, roundPage, goPage, navChange, wheelScroll;
   if (!page || pagelen === 1) {
      return
   }
   if (options.mode) {
   ...
   goPage = function (to) {
      ...
      if (_isLocked || to === indexNow) {
         return
      }
      if (options.continuous) {
         to = roundPage(to, 1)
      } else {
         if (to >= pagelen && stepNow[indexNow] >= stepArr[indexNow] || to < 0 && stepNow[indexNow] === 0) {
            return
         }
      }
      _isLocked = true;
            ...
            case m === "touch":
               if (!browser.addEventListener) {
                  ...
                  var pageIndexMax = pagelen - 1,
                     ...
                     prev, next, setIndex, reset, validReset, move, _interval, _touch, _t;
                  ...
                  touchEvent = {
                     ...
                     move: function (e) {
                        ...
                        if (!isValidMove) {
                           ...
                           if (stepNow[indexNow] + direct >= 0 && stepNow[indexNow] + direct <= stepArr[indexNow]) {
                              return
                           }
                           if (_t) {
                              move(delta)
                        ...
                        switch (options.effect.transform["translate"]) {
                           case "Y":
                              isValidSlide = +duration < 250 && abs.y > 30 || abs.y > pageRange.Y * 0.3;
                              nextDiff = delta.y > 0 ? -1 : 1;
                              break;
                           case "X":
                              isValidSlide = +duration < 250 && abs.x > 30 || abs.x > pageRange.X * 0.3;
                              nextDiff = delta.x > 0 ? -1 : 1;
                              break;
                           default:
                              isValidSlide = +duration < 350 && abs.y + abs.x > 50 || abs.y > pageRange.Y * 0.3 || abs.x > pageRange.X * 0.3;
                        ...
                        if (!isValidSlide || !_t || !checkStep(nextDiff) || options.continuous === false && (!prev && nextDiff === -1 || !next && nextDiff === 1)) {
                           ...
                           if (options.beforeChange && options.beforeChange(indexNow, page[indexNow]) === "stop") {
                              if (prev) {
                                 reset(prev, -1)
                              }
                              if (next) {
